"""System tray icon, menu and small status window (tkinter + pystray)."""
...
try:
    ...
    log.warning("pystray/Pillow not available: %s", exc)
    HAS_TRAY = False
    pystray = None
    item = None
    Image = None
...
def _make_icon_image(size: int = 64):
    """Generate icon: blue circle with 'B' (avoids external .ico dependency)."""
    if Image is None:
        return None
...
class StatusWindow:
    """Small status window (tkinter Toplevel)."""
    ...
    def show(self):
        ...
        self.labels["paired"] = Label(frm, text="Paired: …", anchor="w", justify="left")
        self.labels["connected"] = Label(frm, text="Connected: …", anchor="w")
        self.labels["fix"] = Label(frm, text="Fix: …", anchor="w")
        self.labels["addr"] = Label(frm, text="Address: …", anchor="w", font=("Consolas", 8))
        ...
        Button(btn_frame, text="Close", command=self.hide).pack(side="right", padx=2)

        autorun_frame = Frame(frm)
        autorun_frame.pack(fill="x", pady=(4, 0))
        Button(autorun_frame, text="Enable Autorun", command=self._autorun_enable).pack(side="left", padx=2)
        Button(autorun_frame, text="Disable Autorun", command=self._autorun_disable).pack(side="left", padx=2)

        Label(frm, text="Log (recent events):", font=("Segoe UI", 8)).pack(anchor="w", pady=(8, 0))
        self.log_text = Text(frm, height=8, font=("Consolas", 7), wrap="word")
        self.log_text.pack(fill="both", expand=False)
        self.log_text.insert(END, "Status window opened.\n")
    ...
    def _apply_status(self, st):
        if self.win is None or not self.win.winfo_exists():
            return
        self.labels["paired"].config(text=f"Paired: {'yes' if st.paired else 'no'}")
        self.labels["connected"].config(text=f"Connected: {'yes' if st.connected else 'no'}")
        if st.as_ver is None:
            self.labels["fix"].config(text="Fix: unknown (buds sleeping or no verify)")
        else:
            ok = "allowed" if st.multipoint_allowed else "blocked"
    ...
    def _autorun_enable(self):
        self.append_log("Enabling autorun…")
        def worker():
            try:
                ...
                self.root.after(0, lambda: self.append_log(f"Autorun error: {exc}"))
        threading.Thread(target=worker, daemon=True).start()

    def _autorun_disable(self):
        self.append_log("Disabling autorun…")
        def worker():
            try:
    ...
    def _revert_fix(self):
        if not messagebox.askyesno("Revert fix", "Undo multipoint fix (asVer=0)?\n\nBuds will return to stock behavior."):
            return
        ...
        def worker():
            try:
                ...
                self.root.after(0, lambda: self.append_log(f"Revert {'ok' if ok else 'failed'}: {msg}"))
                self.root.after(0, lambda: self.refresh_now(silent=True))
            except Exception as exc:
                self.root.after(0, lambda: self.append_log(f"Error: {exc}"))
        threading.Thread(target=worker, daemon=True).start()
...
class TrayApp:
    ...
    def _on_fix(self, icon, item):
        def worker():
            dev = buds_fix.find_buds(self.config.get("name_needle", "buds"),
                                     self.config.get("address"))
            if not dev:
                self._notify("No Buds found", "Pair Galaxy Buds first in Bluetooth settings")
                return
            ok, msg = buds_fix.apply_fix(dev.address, self.config.get("channel"),
                                         as_ver=int(self.config.get("as_ver", 2)))
            self._notify("Fix " + ("ok" if ok else "failed"), msg)
            if self.status_win:
    ...
    def _on_revert(self, icon, item):
        def worker():
            dev = buds_fix.find_buds(self.config.get("name_needle", "buds"),
                                     self.config.get("address"))
            if not dev:
                self._notify("No Buds found", "")
                return
            ok, msg = buds_fix.revert_fix(dev.address, self.config.get("channel"))
            self._notify("Revert " + ("ok" if ok else "failed"), msg)
        threading.Thread(target=worker, daemon=True).start()
    ...
    def _build_menu(self):
        return pystray.Menu(
            ...
            item("Enable Autorun", self._on_autorun_enable, enabled=lambda i: self._is_autorun_off(i)),
            item("Disable Autorun", self._on_autorun_disable, enabled=lambda i: self._is_autorun_on(i)),
            pystray.Menu.SEPARATOR,
            item("Uninstall (full)", self._on_uninstall),
            item("Close", self._on_exit),
        )
            ...
            while True:
                ...
                if self.icon:
                    ...
                    if not st.paired:
                        ...
                        tip += f" — {st.name or 'Buds'} not connected"
                    else:
                        tip += f" — {st.name or 'Buds'} connected"
                    try:
                        self.icon.title = tip
                    except Exception:
                        pass
    ...
    def run(self):
        ...
        if not HAS_TRAY:
            log.warning("pystray not available — fallback to status window only")
        ...
        log.info("tray started — right-click icon for menu")
        self.root.mainloop()
